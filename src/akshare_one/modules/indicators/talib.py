...
def _as_f64(series: pd.Series) -> NDArray[np.float64]:
    """Convert a column to the plain float64 array TA-Lib's stubs require.

    pandas-stubs types ``Series.values`` as a union that includes
    ``ExtensionArray``, which is not assignable to ``NDArray[np.float64]``.
    """
